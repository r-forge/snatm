makeforest <- function(month){
  ...
  authors <- c()
  for (i in seq_along(authorlist)){authors <- c(authors,authorlist[[i]][1])}
  headinglist <- sapply(workingobject,"Heading")
  headings <- c()
  for (i in 1:length(headinglist)){headings <- c(headings,headinglist[[i]][1])}
  forest <- cbind(numberofmail
               ,threadid[!is.na(threadid)]
               ,authors
               ,headings
               ,Content[!is.na(threadid)]
               )
  colnames(forest) <- c("emailID","threadID","author","subjects","content")
  forest
}
