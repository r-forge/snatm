...
wn.replace <- function(terms,protect=NULL){
  skip <- integer(0)
  replace.list <- c()
  replace.w <- c()
  remove <- c()
  ...
  if (!is.null(replace.w)){
    ...
    replaced.words <- gsub(replaced.words,pattern="\\[",replacement="")[seq(1,length(replaced.words),by=2)]
    replacement.words <- unlist(strsplit(replace.w,split="\\]\\["))
    replacement.words <- gsub(replacement.words,pattern="\\]",replacement="")[seq(2,length(replacement.words),by=2)]
    ...
    replace.list <- replace.list[replace.list[,1]!=replace.list[,2],]
    ...
    if (nrow(replace.list)!=0){
      for (j in 1:nrow(replace.list)){
        ...
        if (length(r)>1){
          replace.list <- rbind(replace.list,cbind(r,replace.list[j,2]))
          remove <- c(remove,j)
        }
      }
    }
