...
centrality.edgelist <- function(terms,apply.to=c("subjects","content"),list=c("rhelp","rdevel")){
  ...
  for (i in seq_along(terms)){
    if (is.element(file.path(list,apply.to,paste("net_",terms[i],".rda",sep="")),list.files(paste(list,"/",apply.to,sep="")))){
      load(file.path(list,apply.to,paste("net_",terms[i],".rda",sep="")))
      if (dim(net)[1]>19){
