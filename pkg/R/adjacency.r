adjacency <- function(edgelist,directed=TRUE,valued=TRUE,mode="multiple"){
  ...
  if (is.data.frame(edgelist)){
    edgelist1 <- cbind(as.vector(edgelist[,1]),as.vector(edgelist[,2]))
    vnames <- unique(c(as.vector(edgelist1[,1]),as.vector(edgelist[,2])))
  ...
  for (i in 1:dim(edgelist)[1]){
    ...
    adjmatrix[aparty,bparty] <- adjmatrix[aparty,bparty]+value
    if (directed==FALSE){
      adjmatrix[bparty,aparty] <- adjmatrix[bparty,aparty]+value
    }
  }
  if(valued==FALSE){
